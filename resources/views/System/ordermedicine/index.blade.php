                            ...
                            <tbody>
                                @forelse ($ordersUnpaid as $medicine)
                                    @if ($medicine->status == 0)
                                        @php
                                            $totalPrice = $medicine->total_medicine_price ;
                                        @endphp
                ...
                <div class="tab-pane fade " id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab">
                    <div class="table-responsive ">
                        <table class="table table-bordered text-nowrap mb-0 align-middle">
                            ...
                            <tbody>
                                @forelse ($ordersPaid as $medicine)
                                    @if ($medicine->status == 1)
                                        ...
                                        <tr class="order-row text-center">
                                            ...
                                            <td class="border-bottom-0 d-flex justify-content-center align-items-center">
                                               ...
                                               {{-- Form xóa --}}
<form action="{{ route('system.ordermedicine.delete', $medicine->order_medicine_id) }}"
    id="form-delete{{ $medicine->order_medicine_id }}"
    method="POST"
...
{{-- Nút xóa --}}
<button type="button"
    class="btn btn-danger btn-delete me-1"
    data-id="{{ $medicine->order_medicine_id }}">
                    ...
                    <div class="table-responsive ">
                        <div class="table-responsive">
                            <table class="table table-bordered text-nowrap mb-0 align-middle">
                                ...
                                <tbody>
                                    @forelse ($ordersDispensed as $medicine)
                                        @if ($medicine->status == 2)
                                            @php
                                                $totalPrice =
                                                    $medicine->total_medicine_price ;
                                            @endphp
                                            <tr class="order-row text-center">
                                                <td class="border-bottom-0">
                                                    <p class="fw-semibold mb-0">{{ $medicine->order_medicine_id }}</p>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <div class="d-flex flex-row align-items-center">
                            <img src="{{ asset('backend/assets/images/logos/NhatMinh.png') }}" alt="Hospital Logo"
                                style="width: 60px;">
                            <div class="ms-3">
                                <h5 class="mb-1">Bệnh viện Đa Khoa Nhật Minh</h5>
                                <p class="mb-0"><strong>Địa chỉ: </strong>54 Đản Dị - Uy Nỗ - Đông Anh - Hà Nội</p>
        ...
        <script>
            $(document).ready(function() {
                ...
                $('.payment-button').on('click', function() {
                    ...
                    $.ajax({
                        url: '/admin/order-medicines/edit/' + ordermedicineId,
                        type: 'GET',
                        dataType: 'json',
            ...
            `);
                                ...
                                totalMedicineAmount = parseFloat(
    totalMedicineAmount.toString().replace(/[^0-9.-]/g, '')
) || 0;

// Không dùng phí dịch vụ nữa
var totalAmount = totalMedicineAmount;
    ...
    $('#tabInput').val(0); // Chưa thanh toán
}
else if (activeTab === 'nav-contact-tab') {
    $('#tabInput').val(2); // Đã thanh toán
}
else if (activeTab === 'nav-contacts-tab') {
    $('#tabInput').val(3); // Đã phát thuốc
}
                });
