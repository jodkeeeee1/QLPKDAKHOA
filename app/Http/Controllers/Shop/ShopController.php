...
$products = $query->select(
    ...
    $product->isInSalePeriod =
        $product->dateStartSale <= Carbon::now()
        && $product->dateEndSale >= Carbon::now();
    ...
    ->first();
    $productById->img_array = DB::table('img_products')
    ->where('product_id', $productById->product_id)
    ->pluck('img')
    ->toArray();
    ...
    ->limit(4)
    ->get();
$productByCategory->transform(function ($item) {

    $item->img_array = DB::table('img_products')
    ...
    public function addProductToCart($id, Request $request)
{
    ...
    $cartCount = CartDetail::where('cart_id', $cartProduct->cart_id)
        ->sum('quantity');
