...
use App\Http\Requests\Client\Order\OrderRequest;
...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $ordersPrepaid = (clone $ordersquery)
    ->where('orders.status', 1)
    ->paginate(10)
    ->appends($request->query());

$ordersisPrepaid = (clone $ordersquery)
...
{
    $orders = Order::leftJoin('books', 'books.book_id', '=', 'orders.book_id')
        ->leftJoin('specialties', 'specialties.specialty_id', '=', 'books.specialty_id')
        ->leftJoin('schedules', 'schedules.shift_id', '=', 'books.shift_id')
        ->leftJoin('users', 'users.user_id', '=', 'schedules.user_id')
        ->select(
    ...
    $pdf = Pdf::loadView('System.order.pdforderonline', [
        'orders' => $orders
    ]);
