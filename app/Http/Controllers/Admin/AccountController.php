...
class AccountController extends Controller
{
    public function index(Request $request)
    {
        ...
        $admin = $admin->where('role', 1)->orderBy('row_id', 'desc')->paginate(10)->appends($request->query());
             ...
             // Truy vấn admin có role = 1 (Quản trị)
        $doctor = clone $usersQuery; // Tạo bản sao của truy vấn
        $doctor = $doctor->where('role', 2)->orderBy('row_id', 'desc')->paginate(10)->appends($request->query());
        ...
        return view('System.accounts.index', compact('users', 'admin', 'doctor', 'staff' ,'activeTab'));
    }
