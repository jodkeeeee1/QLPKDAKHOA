...
{
    $medicineType = MedicineType::where('medicine_type_id', $row_id)->first();

    if (!$medicineType) {
        return redirect()->back()->with('error', 'Không tìm thấy danh mục thuốc!');
    }

    $medicineType->delete();

    return redirect()->back()->with('success', 'Xóa danh mục thuốc thành công!');
}
}