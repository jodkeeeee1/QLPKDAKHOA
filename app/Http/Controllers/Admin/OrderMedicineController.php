...
class OrderMedicineController extends Controller
{
    public function index(Request $request)
    {
        ...
        $orderMedicinesQuery = OrderMedicine::join('treatment_details', 'order_medicines.treatment_id', '=', 'treatment_details.treatment_id')
           ...
           ->join('medical_records', 'treatment_details.medical_id', '=', 'medical_records.medical_id')
        ...
        $ordersUnpaid = (clone $orderMedicinesQuery)
        ->where('order_medicines.status',0)
        ->paginate(10,['*'],'unpaid_page')
        ->appends($request->query());

    $ordersPaid = (clone $orderMedicinesQuery)
        ->where('order_medicines.status',1)
        ->paginate(10,['*'],'paid_page')
        ->appends($request->query());

    $ordersDispensed = (clone $orderMedicinesQuery)
        ->where('order_medicines.status',2)
        ->paginate(10,['*'],'dispensed_page')
        ->appends($request->query());


          return view('System.ordermedicine.index', compact(
        'ordersUnpaid',
        'ordersPaid',
        'ordersDispensed'
    ));
...
{
    $order = OrderMedicine::where('order_medicine_id', $id)->first();
