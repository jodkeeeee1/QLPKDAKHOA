...
$query = Book::leftJoin('table_shifts', 'table_shifts.shift_id', '=', 'books.shift_id')
    ->where('books.user_id', $user->user_id)
    ->whereIn('books.status', [0, 1, 2])

    // ❌ loại những booking đã khám xong
    ->whereNotExists(function ($q) {
        $q->select(DB::raw(1))
          ->from('medical_records')
          ->whereColumn('medical_records.book_id', 'books.book_id')
    ...
    public function create($book_id, Request $request)
    {
        ...
        $book = Book::where('book_id', $book_id)->first();
           // 🔒 CHẶN CHƯA ĐẾN NGÀY KHÁM
    $today = Carbon::today();

if (Carbon::parse($book->day)->gt($today)) {
    return redirect()->back()->with('error', 'Chưa đến ngày khám');
}
        $phone = $book->phone;
...
$content = Book::join('schedules', 'schedules.shift_id', '=', 'books.shift_id')
    ...
    public function store(CheckupHealthRequest $request, $medical_id)
    {
       ...
       TreatmentMedication::where('treatment_id', $treatment_id)->delete();

$medicines = json_decode(
    $request->input('selectedMedicines'),
    true
);

$oldMedicines = TreatmentMedication::where(
    'treatment_id',
    $treatment_id
)->get();
...
foreach ($oldMedicines as $old) {
    ...
    if ($medicine) {
        $medicine->amount += $old->quantity;
        $medicine->save();
    }
}

TreatmentMedication::where(
    'treatment_id',
    $treatment_id
)->delete();
...
foreach ($medicines as $medicineData) {
    ...
    // kiểm tra tồn kho
    if ($medicine->amount < $medicineData['quantity']) {
        return redirect()->back()
            ->with(
                'error',
                'Thuốc '.$medicine->name.' không đủ số lượng'
            );
    }
    ...
    // trừ kho
    $medicine->amount -= $medicineData['quantity'];
    $medicine->save();
}
        $user = Auth::user();
...
$order_medicine->order_medicine_id =
    strtoupper(Str::random(10));

$order_medicine->treatment_id =
    $treatment_id;

$order_medicine->status = 0;



$order_medicine->total_price =
    $totalPrice;

$order_medicine->user_id =
    $user_id;
...
$order_medicine->save();
    ...
    public function saveService(Request $request, $book_id)
    {
        ...
        if ($request->input('action') === 'cancel') {
           ...
           $medical_recordbook = MedicalRecord::where('book_id', $book_id)
...
$medical_record->user_id = Auth::user()->user_id;
...
$medical_record->save();
            ...
            $treatment = TreatmentDetail::where('medical_id', $medical_record->medical_id)->first();
...
if (!$treatment) {
    ...
    // ❌ XÓA CÁI CŨ TRƯỚC
    TreatmentService::where('treatment_id', $treatment->treatment_id)->delete();
    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($services)) {
        ...
        foreach ($services as $serviceData) {
            ...
            $saveService->service_id = $serviceData['id'];
        ...
        return response()->json(['error' => 'Invalid data format'], 400);
            ...
            $service = Service::get();
            $medicine = Medicine::select('*')->distinct()->get();
            $medical = MedicalRecord::where('book_id', $book_id)
    ->latest()
    ->first();
    ...
    public function saveMedical(Request $request, $book_id)
    {
        ...
        $medical_book = MedicalRecord::where('book_id', $book_id)
